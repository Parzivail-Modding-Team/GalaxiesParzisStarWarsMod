...
final class Decode
{
	...
	{
		int toWrite = Math.min(s.outputLength - s.outputUsed, s.ringBufferBytesReady - s.ringBufferBytesWritten);
		if (toWrite != 0)
		{
				...
				case INSERT_LOOP:
					...
					else
					{
						...
						while (s.j < s.insertLength)
						{
							...
							int literalTreeIndex = s.contextMap[s.contextMapSlice + (Context.LOOKUP[s.contextLookupOffset1 + prevByte1] | Context.LOOKUP[s.contextLookupOffset2 + prevByte2])] & 0xFF;
							s.literalBlockLength--;
							prevByte2 = prevByte1;
							BitReader.fillBitWindow(s);
