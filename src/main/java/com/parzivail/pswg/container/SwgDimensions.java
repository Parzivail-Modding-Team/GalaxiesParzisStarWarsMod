...
public class SwgDimensions
{
	public static class Tatooine
	{
		...
		public static void registerDimension()
		{
			...
			DIMENSION_TYPE = FabricDimensionType.builder().defaultPlacer((oldEntity, destinationWorld, portalDir, horizontalOffset, verticalOffset) -> new BlockPattern.TeleportTarget(new Vec3d(destinationWorld.getTopPosition(Heightmap.Type.WORLD_SURFACE, BlockPos.ORIGIN)), oldEntity.getVelocity(), (int) oldEntity.yaw)).factory(TatooineDimension::new).skyLight(true).buildAndRegister(identifier);
		}
	}
}
