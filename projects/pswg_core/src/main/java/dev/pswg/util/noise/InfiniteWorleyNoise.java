...
public class InfiniteWorleyNoise
{
	...
	private Vec2 R(Vec2 n)
	{
		return new Vec2(R(n.x * 23.62f - 300 + n.y * 34.35f), R(n.x * 45.13f + 256 + n.y * 38.89f));
	}

	private Vec3 R(Vec3 n)
	{
		return new Vec3(R(n.x * 23.62f - 300 + n.y * 34.35f + 663 + n.z * 36.57f), R(n.x * 45.13f + 256 + n.y * 38.89f - 764 + n.z * 91.58f), R(n.x * 13.2f - 458 + n.y * 56.24f + 172 + n.z * 68.45f));
	}

	public double eval(double nx, double ny)
	{
		var n = new Vec2((float)nx, (float)ny);
		var dis = 2d;
		for (float x = -1; x <= 1; x++)
		{
			for (float y = -1; y <= 1; y++)
	...
	{
		var n = new Vec3((float)nx, (float)ny, (float)nz);
		var dis = 2d;
		for (float x = -1; x <= 1; x++)
		{
			for (float y = -1; y <= 1; y++)
