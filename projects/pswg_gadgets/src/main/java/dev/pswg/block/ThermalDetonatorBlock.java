...
public class ThermalDetonatorBlock extends WaterloggableRotatingBlock
{
	...
	{
		if (player.getInventory().getMainHandStack().isOf(Gadgets.THERMAL_DETONATOR_ITEM) && state.get(CLUSTER_SIZE) < MAX_CLUSTER_SIZE && player.isSneaking())
		{
	...
	{
		return new ItemStack(Gadgets.THERMAL_DETONATOR_ITEM);
	}
	...
	{
		var tde = new ThermalDetonatorEntity(Gadgets.THERMAL_DETONATOR_ENTITY, world);
