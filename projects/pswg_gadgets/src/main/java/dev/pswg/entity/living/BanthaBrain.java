package dev.pswg.entity.living;
...
import dev.pswg.entity.living.SwgMemoryModuleTypes;
import dev.pswg.mixin.BreedTaskAccessor;
...
public class BanthaBrain
{
	...
	//TODO find updated method for look and wander tasks
	...
	public static void addCoreActivities(Brain<BanthaEntity> brain)
	{
		brain.setTaskList(
				...
				ImmutableList.of(
						...
						new FleeTask<>(2.5F),
						//new LookAroundTask(45, 90, 0, 0),
						//new WanderAroundTask(),
						new TickCooldownTask(MemoryModuleType.TEMPTATION_COOLDOWN_TICKS),
						new TickCooldownTask(SwgMemoryModuleTypes.CALL_COOLDOWN),
						new TickCooldownTask(SwgMemoryModuleTypes.FORAGE_COOLDOWN)
				)
		);
	}
	...
	public static void addIdleActivities(Brain<BanthaEntity> brain)
	{
		brain.setTaskList(
				Activity.IDLE,
				ImmutableList.of(
						Pair.of(0, new BanthaBreed(LivingEntities.Bantha, 1.0F, 4)),
	...
	public static Ingredient getTemptItems()
	{
		...
		return Ingredient.fromTag(RegistryEntryList.of(RegistryEntry.of((Registries.ITEM.stream().reduce((item, item2) -> item.getDefaultStack().isIn(Gadgets.BANTHA_TEMPT) ? item : Items.AIR)).get())));
	}
	...
	public static class BanthaBreed extends BreedTask
	{
		...
		public BanthaBreed(EntityType<? extends AnimalEntity> targetType, float speed, int distance)
		{
			super(targetType, speed, distance);
			this.speed = speed;
		...
		protected void keepRunning(ServerWorld serverWorld, AnimalEntity animalEntity, long l)
		{
			...
			TargetUtil.lookAtAndWalkTowardsEachOther(animalEntity, animalEntity2, speed, distance);
