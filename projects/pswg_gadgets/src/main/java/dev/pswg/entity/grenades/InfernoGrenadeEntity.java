...
public class InfernoGrenadeEntity extends GrenadeEntity
{
	...
	{
		var world = getWorld();
		world.addParticle(GadgetsParticleTypes.INFERNO_SCORCH_PARTICLE,
		                  true,
		                  true,
						...
						if (!dirList.isEmpty())
						{
							...
							if (!offsetState.isAir() && !offsetState.isLiquid())
							{
								...
								for (Direction dir3 : dirList)
								{
									...
									for (float i = 0; i < (world.random.nextBetween(1, 4) * (MAX_DISTANCE + 1 - getPos().distanceTo(offsetPos2.toCenterPos()))); i += 1)
									{
										...
										world.addParticle(ParticleTypes.SMOKE,
										                  true,
										                  true,
										                  offsetPos.getX() + unitVec.x + offsetX,
										                  offsetPos.getY() + unitVec.y + offsetY,
										                  offsetPos.getZ() + unitVec.z + offsetZ,
										                  0,
									...
									for (float i = 0; i < (world.random.nextBetween(1, 2) * (MAX_DISTANCE + 1 - getPos().distanceTo(offsetPos2.toCenterPos()))) / 2d; i += 1)
									{
										double offsetX = this.random.nextGaussian() * (unitVec.y + unitVec.z);
										double offsetY = this.random.nextGaussian() * (unitVec.x + unitVec.z);
										double offsetZ = this.random.nextGaussian() * (unitVec.y + unitVec.x);
