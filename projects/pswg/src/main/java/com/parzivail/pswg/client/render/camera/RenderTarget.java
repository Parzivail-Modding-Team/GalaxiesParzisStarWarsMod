...
import com.parzivail.pswg.mixin.GameRendererAccessor;
...
public class RenderTarget
{
	...
	public static void capture(float tickDelta, long startTime, boolean tick)
	{
		...
		var previousCamera = client.cameraEntity;
		client.cameraEntity = CAMERA_ENTITY.with(client.world, new Vector3d(-154, 71, 119), new Quaternionf().rotationYXZ(-130 * MathHelper.RADIANS_PER_DEGREE, -18 * MathHelper.RADIANS_PER_DEGREE, 0), true);

		client.gameRenderer.renderWorld(client.getTickDelta(), 0, new MatrixStack());

		client.cameraEntity = previousCamera;
		((GameRendererAccessor)client.gameRenderer).setCamera(camera);
