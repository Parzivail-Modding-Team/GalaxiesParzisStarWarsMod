...
public class HitboxHelper
{
	...
	private static void line(VertexConsumer vertices, MatrixStack ms, Vec3d start, Vec3d end)
	{
		...
		var entry = ms.peek();
		vertices.vertex(entry.getPositionMatrix(), (float)start.x, (float)start.y, (float)start.z).color(255, 255, 0, 255).normal(entry, (float)d.x, (float)d.y, (float)d.z);
		vertices.vertex(entry.getPositionMatrix(), (float)end.x, (float)end.y, (float)end.z).color(255, 255, 0, 255).normal(entry, (float)d.x, (float)d.y, (float)d.z);
	}
}
