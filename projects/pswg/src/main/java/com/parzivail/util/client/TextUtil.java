...
public class TextUtil
{
	...
	public static void drawArea(DrawContext context, TextRenderer renderer, int x, int y, int width, Text text)
	{
		var lines = renderer.wrapLines(text, width);

		for (var m = 0; m < lines.size(); ++m)
			context.drawText(renderer, lines.get(m), x, y + m * renderer.fontHeight, 0, false);
	}
}
